...
const Clients = () => {
  ...
  const handleAddClient = async (e) => {
    ...
    // Verificação do limite de registros
    if (clients.length >= CLIENT_LIMIT) {
      return alert(`Limite de ${CLIENT_LIMIT} clientes atingido.`)
    }
    ...
    <div className="space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white px-6 py-2.5 rounded text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm active:scale-95"
        >
          <UserPlus className="w-4 h-4" />
          Registrar Novo
        </button>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="4" className="px-8 py-20 text-center">
                     <div className="text-slate-200 mb-4 flex justify-center"><Users className="w-12 h-12" /></div>
                     <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">A base de dados retornou consulta vazia.</p>
                     <button onClick={() => setIsModalOpen(true)} className="mt-4 text-[10px] font-bold text-indigo-600 hover:underline uppercase tracking-widest">Adicionar Entrada</button>
                  </td>
                </tr>
              ) : (
                filteredClients.map((client) => (
      ...
      {/* Aviso de Capacidade */}
      {clients.length >= CLIENT_LIMIT && (
        <div className="bg-red-50 border border-red-100 p-4 rounded flex items-center gap-4">
           <AlertCircle className="w-5 h-5 text-red-500 shrink-0" />
           <p className="text-[10px] text-red-700 font-bold uppercase tracking-tight">
             STATUS CRÍTICO: Limite de armazenamento atingido ({CLIENT_LIMIT} registros). Libere slots para novos registros.
           </p>
        </div>
      )}
